...
# Detect the OpenEnv directory in this repository structure.
if [ -f "$REPO_DIR/openenv.yaml" ]; then
  OPENENV_DIR="$REPO_DIR"
elif [ -f "$REPO_DIR/risk_prediction/openenv.yaml" ]; then
  OPENENV_DIR="$REPO_DIR/risk_prediction"
else
  OPENENV_DIR="$REPO_DIR"
fi
...
elif [ -f "$REPO_DIR/risk_prediction/server/Dockerfile" ]; then
...
VALIDATE_OK=false
VALIDATE_OUTPUT="$(cd "$OPENENV_DIR" && openenv validate 2>&1)" && VALIDATE_OK=true
